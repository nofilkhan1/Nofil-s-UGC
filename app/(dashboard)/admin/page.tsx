...
export default async function AdminPage() {
  ...
  return <div className="stack" style={{ gap: "2rem" }}>
    ...
    <section><h2 className="section-title">All brands</h2><div className="data-table-wrap"><table className="data-table"><caption>Brand accounts and campaign totals</caption><thead><tr><th scope="col">Company name</th><th scope="col">Email</th><th scope="col">Campaigns</th></tr></thead><tbody>{overview.brands.length ? overview.brands.map((brand) => <tr key={brand.id}><td><strong>{brand.company_name}</strong></td><td>{brand.email ?? "—"}</td><td>{brand.campaign_count}</td></tr>) : <tr><td colSpan={3}>No brands yet.</td></tr>}</tbody></table></div></section>
    <section><h2 className="section-title">All creators</h2><div className="data-table-wrap"><table className="data-table"><caption>Creator accounts and application totals</caption><thead><tr><th scope="col">Public name</th><th scope="col">Email</th><th scope="col">Applications</th></tr></thead><tbody>{overview.creators.length ? overview.creators.map((creator) => <tr key={creator.id}><td><strong>{creator.public_name}</strong></td><td>{creator.email ?? "—"}</td><td>{creator.application_count}</td></tr>) : <tr><td colSpan={3}>No creators yet.</td></tr>}</tbody></table></div></section>
    <section><h2 className="section-title">All campaigns</h2><div className="data-table-wrap"><table className="data-table"><caption>Campaign status and application totals</caption><thead><tr><th scope="col">Title</th><th scope="col">Brand</th><th scope="col">Status</th><th scope="col">Applications</th></tr></thead><tbody>{overview.campaigns.length ? overview.campaigns.map((campaign) => <tr key={campaign.id}><td><strong>{campaign.title}</strong></td><td>{campaign.brand_name}</td><td><StatusBadge status={campaign.status} /></td><td>{campaign.application_count}</td></tr>) : <tr><td colSpan={4}>No campaigns yet.</td></tr>}</tbody></table></div></section>
  </div>;
}
