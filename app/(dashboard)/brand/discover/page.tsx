...
export default async function BrandDiscoverPage() {
  ...
  const { data: details } = creatorIds.length ? await supabase.from("creator_profiles").select("user_id,bio,portfolio_url,instagram_handle,tiktok_handle,top_content_links,niches").in("user_id", creatorIds) : { data: [] };
