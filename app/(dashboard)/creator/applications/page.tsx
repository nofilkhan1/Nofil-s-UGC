...
type CreatorApplication = Application & {
  campaign: Campaign & { brand: { display_name: string } | null };
};
...
export default async function CreatorApplicationsPage({
  ...
  const { data, error } = await supabase
    .from("applications")
    .select(
      "*, campaign:campaigns!applications_campaign_id_fkey(*, brand:profiles!campaigns_brand_id_fkey(display_name))",
    )
  ...
  return (
    <div className="stack" style={{ gap: "1.7rem" }}>
      <div>
        ...
        <p className="muted">
          Track every quote and brand decision in one place.
        </p>
      </div>
      {query.submitted === "1" ? (
        <div className="notice notice--success" role="status">
          Application sent. The brand can now review your profile and quote.
        </div>
      ) : null}
      {error ? (
        ...
        <EmptyState
          ...
          message="Browse live campaigns and apply when the brief fits your work."
          action="Find campaigns"
          ...
          {applications.map((application) => (
            <article className="panel" key={application.id}>
              ...
              >
                <div>
                  <span
                    className={`badge badge--${application.campaign.platform}`}
                  >
                  ...
                  <p className="muted">
                    {application.campaign.brand?.display_name} ·{" "}
                    {application.campaign.post_count} ×{" "}
                    {application.campaign.content_format}
                  </p>
                </div>
              ...
              >
                <span>
                  <small className="muted">Your quote</small>
                  <strong
                    style={{
                      display: "block",
                  ...
                  >
                    ...
                    {Number(application.price_per_post).toLocaleString("en")}{" "}
                    per post
                  </strong>
                </span>
                <span>
                  <small className="muted">Campaign dates</small>
                  <strong style={{ display: "block" }}>
                    {application.campaign.start_date} –{" "}
