...
export default async function CreatorCampaignDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabase.from("campaigns").select("*, brand:profiles!campaigns_brand_id_fkey(display_name)").eq("id", id).eq("status", "published").single();
  if (!data) notFound();
