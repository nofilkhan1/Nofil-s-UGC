...
export default async function CreatorCampaignsPage({ searchParams }: { searchParams: Promise<{ platform?: string; niche?: string | string[] }> }) {
  ...
  const { data: creatorProfile } = await supabase.from("creator_profiles").select("niches").eq("user_id", viewer.user.id).maybeSingle();
  const creatorNiches = (creatorProfile?.niches ?? []) as Niche[];
  ...
  const campaigns = rows.map((campaign) => ({ ...campaign, application_count: campaign.applications?.[0]?.count ?? 0, niche_match_count: creatorNiches.length ? countNicheMatches(campaign.niches ?? [], creatorNiches) : undefined, appliedStatus: appliedByCampaign.get(campaign.id) as "pending" | "approved" | "rejected" | undefined }));
  const filterHref = (platform: Platform | null, niches: readonly Niche[]) => { const nextParams = new URLSearchParams(platform ? { platform } : {}); niches.forEach((item) => nextParams.append("niche", item)); return `/creator/campaigns${nextParams.size ? `?${nextParams}` : ""}`; };
  const nicheHref = (niche: Niche) => filterHref(activePlatform, activeNiches.includes(niche) ? activeNiches.filter((item) => item !== niche) : [...activeNiches, niche]);
  return <div className="stack" style={{ gap: "1.7rem" }}><div><p className="eyebrow">Creator opportunities</p><h1 className="page-title">Find your next brief</h1><p className="muted">Review every requirement first. Apply only when the format, dates, and product fit your work.</p></div><nav className="cluster" aria-label="Filter campaigns by platform"><Link className={`button ${!activePlatform ? "button--primary" : "button--secondary"}`} href={filterHref(null, activeNiches)}>All platforms</Link><Link className={`button ${activePlatform === "instagram" ? "button--primary" : "button--secondary"}`} href={filterHref("instagram", activeNiches)}>Instagram</Link><Link className={`button ${activePlatform === "tiktok" ? "button--primary" : "button--secondary"}`} href={filterHref("tiktok", activeNiches)}>TikTok</Link></nav><nav className="niche-badges" aria-label="Filter campaigns by category">{NICHES.map((niche) => <Link className={`badge ${activeNiches.includes(niche) ? "badge--active" : ""}`} href={nicheHref(niche)} key={niche}>{niche}</Link>)}</nav>{error ? <div className="notice notice--error" role="alert">Campaigns could not be loaded. Refresh the page to try again.</div> : <CampaignBrowseResults campaigns={campaigns} creatorNiches={creatorNiches} />}</div>;
}
