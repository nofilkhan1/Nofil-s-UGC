...
export default async function CreatorProfilePage() {
  ...
  const details = (data ?? { user_id: viewer.user.id, gender: null, age: null, bio: null, portfolio_url: null, instagram_url: null, tiktok_url: null, instagram_handle: null, tiktok_handle: null, niches: [] }) as CreatorProfile;
  return <div className="stack" style={{ maxWidth: "52rem", gap: "1.5rem" }}><div><p className="eyebrow">Creator profile</p><h1 className="page-title">Make your work easy to assess</h1><p className="muted">Brands see these details only after you apply to one of their campaigns.</p>{details.instagram_handle || details.tiktok_handle ? <div className="cluster" style={{ marginTop: "0.75rem" }}>{details.instagram_handle ? <a className="button button--secondary" href={`https://instagram.com/${details.instagram_handle}`} target="_blank" rel="noreferrer">Instagram @{details.instagram_handle}</a> : null}{details.tiktok_handle ? <a className="button button--secondary" href={`https://tiktok.com/@${details.tiktok_handle}`} target="_blank" rel="noreferrer">TikTok @{details.tiktok_handle}</a> : null}</div> : null}</div><section className="panel"><CreatorProfileForm profile={viewer.profile} details={details} /></section></div>;
}
