  ...
  return (
    <div className="app-shell">
      <aside className="app-nav">
        <BrandMark href={profile.role === "brand" ? "/brand/dashboard" : profile.role === "creator" ? "/creator/campaigns" : "/admin"} />
        <nav className="nav-list" aria-label="Main navigation">
          {roleNavigation[profile.role].map(({ href, label, icon: Icon }) => (
            <Link className="nav-link" href={href} key={href}><Icon size={18} aria-hidden="true" />{label}{label === "Notifications" && unread > 0 ? <span className="notification-count" aria-label={`${unread} unread notifications`}>{unread > 99 ? "99+" : unread}</span> : null}</Link>
          ))}
        </nav>
      </aside>
