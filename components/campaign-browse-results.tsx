...
export function CampaignBrowseResults({ campaigns }: { campaigns: CampaignWithBrowseMeta[] }) {
  ...
  return <>
    ...
    {visible.length === 0 ? <EmptyState title={search ? "No campaigns match your search" : "No matching campaigns right now"} message={search ? "Try a different title or description." : "Try another category or platform."} /> : <><p className="muted" role="status">Showing {visible.length} campaign{visible.length === 1 ? "" : "s"}</p><div className="campaign-grid">{visible.map((campaign) => <CampaignCard key={campaign.id} campaign={campaign} href={`/creator/campaigns/${campaign.id}`} />)}</div></>}
  </>;
}
